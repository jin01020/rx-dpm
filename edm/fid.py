...
def calculate_inception_stats(
    ...
    if not image_path.endswith('npz'):
        ...
        images_np = np.load(image_path)['arr_0']
        images_np = images_np.transpose(0, 3, 1, 2)
        dataset_obj = torch.utils.data.TensorDataset(torch.from_numpy(images_np))
    ...
    for datas in tqdm.tqdm(data_loader, unit='batch', disable=(dist.get_rank() != 0)):
        torch.distributed.barrier()
        images = datas[0]
        if images.shape[0] == 0:
            continue
